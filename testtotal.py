...
from data import VOCAnnotationTransform, VOCDetection, BaseTransform, VOC_CLASSES, ImgDataset
    ...
    for i in range(num_images):
        ...
        for i in range(1, detections.size(1)):
            ...
            while detections[0, i, j, 0] >= 0.6:
                ...
                # ocr识别ctc
                # crnn进行解码操作
                #temp = []; temp.extend([0]); temp.extend(detections[0, i, j, 1:].cpu().numpy().tolist())
                #temp = np.expand_dims(temp, 0)
                #rois = torch.from_numpy(np.asarray(temp, dtype=np.float32))
...
def test_voc():
    ...
    ocrnet = OcrLoss()
    load_name = './weights/ssd300_OCR_33000.pth'
