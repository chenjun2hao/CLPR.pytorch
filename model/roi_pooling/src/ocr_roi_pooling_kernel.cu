    ...
    const float* bottom_rois, float* top_data, int* argmax_data)
{
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        ...
        int roi_start_w = round(bottom_rois[n * 5 + 1] * spatial_scale * 1.0/16);
        int roi_start_h = round(bottom_rois[n * 5 + 2] * spatial_scale * 1.0/16);
        int roi_end_w = round(bottom_rois[n * 5 + 3] * spatial_scale * 1.0/16);
        int roi_end_h = round(bottom_rois[n * 5 + 4] * spatial_scale * 1.0/16);
        ...
        if (pw > rois_pooled_width){                                 // 超出的部分之后为0
            top_data[index] = 0;
            if (argmax_data != NULL)
...
__global__ void ROIPoolBackward(const int nthreads, const float* top_diff,
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads)
    {
        ...
        for (int roi_n = 0; roi_n < num_rois; ++roi_n)                  // 多个rois的gradient累加起来
        {
            const float* offset_bottom_rois = bottom_rois + roi_n * 5;  // 索引第i个rois
            ...
            int roi_start_w = round(offset_bottom_rois[1] * spatial_scale * 1.0/16);
            int roi_start_h = round(offset_bottom_rois[2] * spatial_scale * 1.0/16);
            int roi_end_w = round(offset_bottom_rois[3] * spatial_scale * 1.0/16);
            int roi_end_h = round(offset_bottom_rois[4] * spatial_scale * 1.0/16);

            // Skip if ROI doesn't include (h, w)
            const bool in_roi = (w >= roi_start_w && w <= roi_end_w &&              // w表示计算feature的梯度的宽度的索引
            ...
            int rois_pooled_width = (int)(ceil((float)(pooled_height * roi_width) / (float)(roi_height) ));          // 等比例池化，减小
            float bin_size_h = (float)(roi_height) / (float)(pooled_height);
            float bin_size_w = (float)(roi_width) / (float)(rois_pooled_width);         // 会增大

            int phstart = floor((float)(h - roi_start_h) / bin_size_h);                 
            int phend = ceil((float)(h - roi_start_h + 1) / bin_size_h);
            int pwstart = floor((float)(w - roi_start_w) / bin_size_w);                 // 会减小
            ...
            for (int ph = phstart; ph < phend; ++ph) {
                for (int pw = pwstart; pw < pwend; ++pw) {
                    if (offset_argmax_data[(c * pooled_height + ph) * rois_pooled_width + pw] == index)
                    {
                        gradient += offset_top_diff[(c * pooled_height + ph) * rois_pooled_width + pw];
                    }
                }
            }
        }
