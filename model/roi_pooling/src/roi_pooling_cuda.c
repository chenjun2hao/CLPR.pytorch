...
int roi_pooling_forward_cuda(int pooled_height, int pooled_width, float height_spatial_scale, float width_spatial_scale,
                        THCudaTensor * features, THCudaTensor * rois, THCudaTensor * output, THCudaIntTensor * argmax)
{
    ...
    ROIPoolForwardLaucher(
        data_flat, height_spatial_scale, width_spatial_scale,
        num_rois, data_height,
...
int roi_pooling_backward_cuda(int pooled_height, int pooled_width, float height_spatial_scale, float width_spatial_scale,
                        THCudaTensor * top_grad, THCudaTensor * rois, THCudaTensor * bottom_grad, THCudaIntTensor * argmax)
{
    // top_grad为反向传回来的梯度
    // argmax和top_grad应该是一样的维度
    // bottom_grad为特征的梯度
    // Grab the input tensor
    ...
    // }
    // data height
    int data_height = THCudaTensor_size(state, bottom_grad, 2);         // 特征的高度
    // data width
    int data_width = THCudaTensor_size(state, bottom_grad, 3);          // 特征的宽度
    // Number of channels
    int num_channels = THCudaTensor_size(state, bottom_grad, 1);

    cudaStream_t stream = THCState_getCurrentStream(state);
    ROIPoolBackwardLaucher(
        top_grad_flat, height_spatial_scale, width_spatial_scale,
        batch_size, num_rois, data_height,
        data_width, num_channels, pooled_height,
        pooled_width, rois_flat,
