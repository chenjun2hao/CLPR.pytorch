...
__global__ void ROIPoolForward(const int nthreads, const float* bottom_data,
    const float height_spatial_scale, const float width_spatial_scale,
    ...
    const float* bottom_rois, float* top_data, int* argmax_data)
{
    CUDA_KERNEL_LOOP(index, nthreads)
    {
        ...
        // n /= pooled_height;
        // int c = n % channels; spatial_scale
        // n /= channels;
        ...
        // bottom_rois += n * 5;索引rois
        int roi_batch_ind = bottom_rois[n * 5 + 0];
        int roi_start_w = round(bottom_rois[n * 5 + 1] * width_spatial_scale );
        int roi_start_h = round(bottom_rois[n * 5 + 2] * height_spatial_scale );
        int roi_end_w = round(bottom_rois[n * 5 + 3] * width_spatial_scale );
        int roi_end_h = round(bottom_rois[n * 5 + 4] * height_spatial_scale );
        ...
        float bin_size_w = (float)(roi_width) / (float)(rois_pooled_width);         // bin_size_w变大
        ...
        if (in_roi) {                                 // 超出的部分填充为0
            ...
            for (int h = hstart; h < hend; ++h) {
                for (int w = wstart; w < wend; ++w) {
                    ...
                    if (bottom_data[bottom_data_offset + bottom_index] > maxval) {          //bottom：feature 特征
...
int ROIPoolForwardLaucher(
    const float* bottom_data, const float height_spatial_scale, const float width_spatial_scale,
    ...
    float* top_data, int* argmax_data, cudaStream_t stream)
{
    ...
    ROIPoolForward<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
      output_size, bottom_data, height_spatial_scale, width_spatial_scale,
...
__global__ void ROIPoolBackward(const int nthreads, const float* top_diff,
    const int* argmax_data, const int num_rois, const float height_spatial_scale, const float width_spatial_scale,
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads)
    {
        ...
        for (int roi_n = 0; roi_n < num_rois; ++roi_n)
        {
            ...
            int roi_start_w = round(offset_bottom_rois[1] * width_spatial_scale );
            int roi_start_h = round(offset_bottom_rois[2] * height_spatial_scale );
            int roi_end_w = round(offset_bottom_rois[3] * width_spatial_scale );
            int roi_end_h = round(offset_bottom_rois[4] * height_spatial_scale );
            ...
            int rois_pooled_width = ceil((float)(pooled_height * roi_width) / (float)(roi_height) );
            float bin_size_h = (float)(roi_height) / (float)(pooled_height);
            float bin_size_w = (float)(roi_width) / (float)(rois_pooled_width);
            ...
            pwstart = fminf(fmaxf(pwstart, 0), rois_pooled_width);
            pwend = fminf(fmaxf(pwend, 0), rois_pooled_width);
...
int ROIPoolBackwardLaucher(const float* top_diff, const float height_spatial_scale, const float width_spatial_scale,
    ...
    float* bottom_diff, const int* argmax_data, cudaStream_t stream)
{
    ...
    ROIPoolBackward<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
      output_size, top_diff, argmax_data, num_rois, height_spatial_scale, width_spatial_scale,
      height, width, channels, pooled_height,
